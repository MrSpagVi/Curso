---
title: Ramas · 7 puertas
---

# Las 7 ramas

!!! warning "Las ramas son para el mes 4 en adelante"
    Esta página es **referencia**. Las ramas se eligen en el [Checkpoint mes 4](../checkpoint-mes-4.md), después del **tronco común** (setup notas + Fase 0 + 2 libros). Si recién empezás, [arrancá por el tour](../empieza-aqui.md) — no por acá.

Siete maneras de entrar al mismo corpus. La bibliografía no se duplica: cada rama reordena lo que ya está en el [plan integrado](../plan/maestro.md), agrega lo que su disciplina específica exige, y propone una secuencia con énfasis propio. Si terminás las 7 hiciste el plan integrado entero por puertas distintas.

[:material-graph: Ver mapa visual del conocimiento](../mapa.md){ .md-button .md-button--primary }

---

## :material-help-circle: ¿Cuál elijo? (si ya estás en el mes 4)

| Si te interesa principalmente... | Vas a... | Duración aprox. |
|---|---|---|
| Sistemas de pensamiento y vocabulario filosófico riguroso | [📚 Filosofía](filosofia.md) | 22-24 meses |
| Cómo funcionan Estados, regímenes, partidos | [🏛 Ciencias Políticas](ciencias-politicas.md) | 18-22 meses |
| Cómo se reparte la riqueza, debates de macro argentina | [💰 Economía Política](economia.md) | 14-18 meses |
| Estado de bienestar, AUH, políticas públicas comparadas | [🤝 Política Social](politica-social.md) | 6-8 meses |
| Cultura como campo de poder, guerra cultural, plataformas | [🎭 Estudios Culturales y Comunicación](estudios-culturales.md) | 6 meses |
| Orden social, clases, instituciones, vida cotidiana | [👥 Sociología](sociologia.md) | 6-8 meses |
| Ucrania, Taiwán, BRICS, geopolítica, sistema mundial | [🌐 Relaciones Internacionales](relaciones-internacionales.md) | 6 meses |
| Recorrer **todo** con cruces deliberados | [📖 Plan integrado](../plan/maestro.md) | 27-29 meses + Ciclo III |

---

## :material-school: Cómo se usa una rama

Una vez que elegiste tu rama en el [checkpoint mes 4](../checkpoint-mes-4.md): la recorrés lineal de bloque 1 a bloque N. **Una sola rama a la vez** (no en paralelo — solo si tenés 8-10 h/sem firmes, NO recomendado).

Cuando terminés esa rama, podés:

- **Sumar otra rama** (ya tenés el tronco común, vas más rápido).
- **Volver al plan integrado** y hacer las fases que te faltan.
- **Pasar al Ciclo III** (política social + cultura + tesina) si terminaste rama y querés producir texto propio largo.

**Recomendaciones por nivel de entrada:**

- Sin base previa: [Política Social](politica-social.md) o [Sociología](sociologia.md) (más empíricas, casos concretos).
- Con tiempo y ganas de intensidad: [Economía](economia.md) o [Filosofía](filosofia.md).

---

## :material-link-variant: Lo común a las 7 ramas

Sin importar qué rama elijas, esto corre en paralelo desde día 1:

- **[Cómo se aprende acá](../como-se-aprende.md)** — ciclo semanal + 5 prompts de IA + reglas de uso. Léelo antes de empezar cualquier rama.
- **[Setup del sistema de notas](../plan/notas.md)** — 1 mes antes. Sin esto, en 12 meses recordás el 10% de lo leído.
- **M1 — Oratoria** — 30-45 min/sem.
- **[Manual de falacias](../plan/falacias.md)** — captura diaria.
- **[Cómo ejecutar](../plan/ejecutar.md)** — cadencia + autoevaluación Bloom 1-6.
- **[Salidas honorables](../plan/salidas-honorables.md)** — qué ganaste si no terminás cada rama.

---

## :material-information: Por qué este modelo y no "cursos separados"

La tentación es replicar la estructura universitaria: un departamento por disciplina, bibliografía separada, módulos estancos. La rechazamos por dos razones:

1. **Es lo que limita a la universidad, no lo que la potencia.** Los autores serios cruzan: Bourdieu es sociólogo, filósofo y economista de la cultura al mismo tiempo; Marx es economista, filósofo, historiador. Separarlos en cursos los empobrece.
2. **Los problemas reales son temas-bisagra.** "Desigualdad" es dato económico **+** problema político **+** naturalización cultural — no se entiende desde un solo eje. Por eso hay [temas-bisagra explícitos](../plan/temas-bisagra.md) que cruzan las 7 ramas.

Las ramas son **puertas** distintas al mismo edificio. Adentro, todo dialoga.

---

[:material-arrow-left: Volver al inicio](../index.md){ .md-button }
[:material-map: Ver plan integrado](../plan/maestro.md){ .md-button }
